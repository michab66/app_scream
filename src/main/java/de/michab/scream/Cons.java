/* $Id: Cons.java 190 2009-07-01 20:53:40Z Michael $
 *
 * Scream / Kernel
 *
  ...
  extends FirstClassObject
{
  ...
  /**
   * Select the <code>eq?</code> procedure for the comparison.
   */
  ...
  /**
   * Select the <code>eqv?</code> procedure for the comparison.
   */
  ...
  /**
   * Select the <code>equal?</code> procedure for the comparison.
   */
   ...
   */
  private FirstClassObject memx( FirstClassObject obj, int cmpFct )
    throws RuntimeX
  {
    ...
    for ( long i = 0 ; i < thisLength ; i++ )
    {
      ...
      if ( ( cmpFct == EQ && eq( obj, subListsCar ) ) ||
           ( cmpFct == EQV && eqv( obj, subListsCar ) ) ||
           ( cmpFct == EQUAL && equal( obj, subListsCar ) ) )
        return subList;
    }
   ...
   * @see #EQUAL
   */
  private FirstClassObject assx( FirstClassObject obj, int cmpFct )
    throws RuntimeX
  {
    ...
    for ( long i = 0 ; i < thisLength ; i++ )
    {
      FirstClassObject subList = (Cons)listRef( i );
